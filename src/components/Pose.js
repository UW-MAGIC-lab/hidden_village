...
const Pose = (props) => {
  ...
  const drawArms = (landmarks, g) => {
    const torsoLandmarks = (({ RIGHT_SHOULDER, LEFT_SHOULDER, SOLAR_PLEXIS }) => ({ RIGHT_SHOULDER, LEFT_SHOULDER, SOLAR_PLEXIS }))(POSE_LANDMARKS);
  }
  ...
  const drawFace = (poseData, g) => {
    let faceOvalCoords = FACEMESH_FACE_OVAL.map((indexPair) => {
      ...
      coordinates.x = (coordinates.x * props.width * .2) - (props.width * .2);
      coordinates.y = (coordinates.y * props.height * .2) - (props.height * .2);;
      return coordinates
    });
  ...
  const drawAbdomen = (poseData, g) => {
    const abdomenLandmarks = (({ PELVIS, LEFT_HIP }) => ({ PELVIS, LEFT_HIP }))(POSE_LANDMARKS);
    ...
    const radius = Math.sqrt(((abdomen.PELVIS.x - abdomen.LEFT_HIP.x) ** 2) + ((abdomen.PELVIS.y - abdomen.LEFT_HIP.y) ** 2))//*0.8
    g.beginFill(0xff3300);
